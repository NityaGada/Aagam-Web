...
export default function Customize() {
    ...
    const { type_name, pattern } = useParams();
    ...
    return <>
        <div style={{ width: '100%', display: 'flex', marginTop: 1 + 'em' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: 1 + 'em' }}>
                <Subtypes items_list = {reqiured_data.current.subtypes} on_subtypes_select = {handle_subtypes_select.bind(this)} />
            </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', margin: '1em 0' }}>
            <Theme items_list = {reqiured_data.current.patterns} on_patterns_select = {(idx) => handle_patterns_select(idx)} />
        </div>
    </>
}